...
router.get('/:id/accounts/user', verifyToken, verifyRole(['usuario', 'admin']), async (req, res) => {
  try {
    const { user, accounts } = await usersController.getAccounts(req.params.id);
...
router.get('/:id/accounts/summary', verifyToken, verifyRole(['usuario', 'admin']), async (req, res) => {
  try {
